...
\begin{itemize}
	...
	\item $\lambda_0, \lambda_1, \lambda_2, \lambda_3 \geq 0$ are nonnegative
	      hyperparameters.
\end{itemize}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		%\small
		\algrenewcommand\algorithmicrequire{\textbf{Before the algorithm:}}
		\Require Compute diagonal of $HH^\top$: $D_{HH} = \sum_{f=1}^{F} M_f^2 \in \mathbb{R}^{M\times N}$
		\algrenewcommand\algorithmicrequire{\textbf{During the algorithm:}}
		\Require
...
\mypar{Updating $\bm{\Theta}, \bm{\Gamma}$, $\bm{\Delta}$, and $\bm{\Lambda}$}
The updates of the dual variables
in~\eqref{eq:admm_applied_lagrangetheta}-\eqref{eq:admm_applied_lagrangelambda}
...
\begin{align*}
	...
	\\
	{\Lambda'}^{k+1} & = {\Lambda'}^k + X^{k+1} - B^{k+1} - V^{k+1}\,.
\end{align*}
