...
objective2 = cp.Minimize(cp.sum_squares(V2) 
                         + Gamma.T@(S - V2)
                         + 0.5*Delta.T@(X - V2 + (1/rho)*Lambda - (1/rho)*Delta - L)
                         - 0.5*Lambda.T@(L + (1/rho)*Lambda - (1/rho)*Delta - X + V2)
                         + (rho/2)*cp.sum_squares(S - V2) 
                         + (rho/8)*cp.sum_squares(X - V2 + (1/rho)*Lambda - (1/rho)*Delta - L) 
                         + (rho/8)*cp.sum_squares(L + (1/rho)*Lambda - (1/rho)*Delta - X + V2)
                         )
